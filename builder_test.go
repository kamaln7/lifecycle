	...
	when("#Build", func() {
		when("building succeeds", func() {
			it("should provide a subset of the build plan to each buildpack", func() {
				builder.Plan = buildpack.BuildPlan{
					Entries: []buildpack.BuildPlanEntry{
						{
				...
				expectedPlanA := buildpack.BuildpackPlan{Entries: []buildpack.Require{
				...
				expectedPlanB := buildpack.BuildpackPlan{Entries: []buildpack.Require{
					{Name: "some-unmet-dep", Version: "v2"},
